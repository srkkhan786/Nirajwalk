...
TELEGRAM_BOT_TOKEN = '7719973366:AAFQA4pATt_Ihky8-q37hfiaTRPn-TzxORI'
ADMIN_USER_ID = 1232047106 
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*\nभोसड़ीके /get_id आईडी 𝐨𝐰𝐧𝐞𝐫 को भेजो 💀\nDM TO BUY ACCESS:- @OWNERSRK 🗿😈💥\n\nTelegram :-https://t.me/ConfigOfSRK*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    text=(
        ...
        f"*मुट्ठ मारी टट्टे हिले*\n\n"
        f"*🅃🄰🅁🄶🄴🅃 : {ip}*\n"
        f"*🄿🄾🅁🅃 : {port}*\n"
        f"*🅃🄸🄼🄴: {duration} 𝓢𝓮𝓬𝓸𝓷𝓭𝓼*\n"
        f"*🄼🄴🅃🄷🄾🄳 : 𝓥𝓲𝓹 𝓤𝓼𝓮𝓻 𝓸𝓯  @OWNERSRK*"
    ),
    parse_mode='Markdown'
)
...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        ...
        await context.bot.send_message(chat_id=chat_id, text="*अटैक खत्म हो गया है चूतिये, दूसरा मार ले🙆🏾‍♂️😅*\n\nकोन से कलर की चड्ढी पहने हो💀🤣*Thank you for using our service!*", parse_mode='Markdown')
